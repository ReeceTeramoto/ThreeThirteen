 ...
 * @author Reece Teramoto (edited to add some methods in)
 * @version (original) July 2013
 * @version (edited) October 2014
 * @version 12/5/14
 *
 */
public class Deck implements Serializable {
	...
	/**
	 * constructor, making a deck from a given array list of cards
	 */
	public Deck(ArrayList<Card> c)
	{
		cards = (ArrayList<Card>) c.clone();
	}
	...
	public void moveTopCardTo(Deck targetDeck) {
		...
		// indivisibly check the deck for empty, and the card, to
	 ...
	 * there is a card in that position, but that it is face-down.
	 */
	public void nullifyDeckExceptTopCard() {
		// synchronize so that we don't get any race conditions (e.g., with
		// shuffle()
		synchronized (this.cards) {
			// null out each card
	...
	/*
	 * Swaps two cards at the given positions in the array list
	 */
	public void swapCards(int pos1, int pos2)
	{
		Card temp1 = cards.get(pos1);
		Card temp2 = cards.get(pos2);
		cards.remove(pos1);
		cards.add(pos1, temp2);
		cards.remove(pos2);
		cards.add(pos2, temp1);
	}
	
	/**
	 * moves card at pos1 to pos2 and shifts all cards from pos2 
	 * and on to the right (inserting a card in front of card at pos2)
	 * @param pos1 index of card being moved
	 * @param pos2 where the card is going
	 */
	public void insertCard(int pos1, int pos2)
	{
		if(pos1 >= pos2)
		{
			Card temp1 = cards.get(pos1);
			
			//shifts any elements after pos1 to the left (subtracts one from their indices)
			cards.remove(pos1);
			
			//inserts temp1 at pos2 and shifts the element currently at that position
		...
		if(pos1<pos2)
		{
			...
			//now the card originally at pos2 is at pos2-1
			cards.add(pos2-1, temp1);
			
		}
	}
	
	/*
	 * Returns the array list of the cards in the deck
	 */
	...
	/*
	 * Adds a card to the first null spot in the deck
	 */
	public void addToFirstNullSpot(Card c)
	{
		//look through the cards array list
		for (int i = 0; i < cards.size(); ++i)
		{
			//when you find the first null entry, set it to be the
			//card that is passed in
			if (cards.get(i) == null)
			{
				cards.set(i, c);
		...
		//if no entries are null, add the card regularly
		add(c);
	}
}
