...
public abstract class LocalGame implements Game, Tickable {
	...
	private final void checkAndHandleAction(GameAction action) {
		...
		if (!canMove(playerId)) {;
			...
			Log.i("LocalGame", "Sent NotYourTurnInfo");
			return;
		}
		...
		if (!makeMove(action)) {
			...
			Log.i("LocalGame", "Sent IllegalMoveInfo");
			return;
		}
		...
		Log.i("LocalGame", "Successfully checked handled action");
	}
	...
	public final void sendAction(GameAction action) {
		Log.i("LocalGame", "sendAction called in LocalGame");
		if (myHandler == null) 
		{	
			Log.i("LocalGame", "There is no handler for sendAction");
			return; // give up if no handler
		}
		// package the action into a message and send it to the handler
		Message msg = new Message();
		msg.obj = action;
		myHandler.dispatchMessage(msg);
		Log.i("LocalGame", "dispatched message after sendAction");
	}
